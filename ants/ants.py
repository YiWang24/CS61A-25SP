    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if existing_ant.can_contain(self):
                existing_ant.store_ant(self)
            elif self.can_contain(existing_ant):
                self.store_ant(existing_ant)
                place.ant = self
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while near_place is not None and not near_place.is_hive:
            if self.lower_bound <= distance <= self.upper_bound and near_place.bees:
                return random.choice(near_place.bees)
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            if not hasattr(target, 'slow_duration'):
                target.slow_duration = 5
                original_action = target.action

                def slow_action(gamestate):
                    if target.slow_duration > 0:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        damage = max(self.damage - 0.25 * distance,0)
        for _ in range(self.insects_shot):
            damage = max(damage - 0.0625,0)
        return damage
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def scare(self, length):
        ...
        """
        if self.is_scared or self.scared_turns > 0:
            return
        ...
        def scared_action(gamestate):
            if self.scared_turns > 0:
                ...
                # This is the important fix - we need to check if it's an even time
                # when the bee is slowed, otherwise don't decrease scared_turns on odd times
                if hasattr(scared_action, 'is_slowed') and gamestate.time % 2 == 1:
                    pass  # On odd turns when slowed, do nothing
                else:
                    if self.place.ant:
                        self.place.ant.reduce_health(self.damage)
                    if back_place and not back_place.is_hive:
        ...
        scared_action.is_slowed = getattr(self.action, 'is_slowed', False)  # Preserve the slowed status
        self.is_scared = True
