...
def about(keywords):
    ...
    def check_keywords(paragraph):
        clean_words = remove_punctuation(paragraph)
        word_list = split(clean_words)
        word_list = [lower(word) for word in word_list]
...
def accuracy(typed, source):
    ...
    correct_count = sum(1 for i in range(min(len(typed_words),len(source_words)))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    return word_list[min_index] if min_difference <= limit else typed_word
...
def furry_fixes(typed, source, limit):
    ...
    def furry_fixes_helper(typed, source, diff):
        if diff > limit:
            return limit + 1
        if not typed:
        ...
        if typed[0] != source[0]:
            return furry_fixes_helper(typed[1:], source[1:],diff + 1)
        return  furry_fixes_helper(typed[1:], source[1:], diff)
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] != source[i] or i >= len(source):
            break
        correct_count += 1
...
def fastest_words(words_and_times):
    ...
    for i in range(len(words)):
        ...
        for j in range(len(times)):
            time_taken = get_time(times, j ,i)
            if time_taken < min_time:
                min_time = time_taken
                fastest_player = j
        fastest[fastest_player].append(words[i])
    return fastest
    # END PROBLEM 10
